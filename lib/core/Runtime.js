...
class Runtime {
  ...
  enqueueWaitingProcess(proc) {
    return new Promise(resolve => {
      this._waitingProcesses.set(proc, resolve);
    });
  }
  ...
  async _tick() {
    ...
    while (currProc) {
      ...
      if (currProc.status !== ProcessStatus.Closed) {
        ...
        if (this._waitingProcesses.has(currProc)) {
          this._waitingProcesses.get(currProc).call(null);
          this._waitingProcesses.delete(currProc);
        } else {
          currProc.execute();
        }
        ...
        } else if (procState === ProcState.HasData && (currProc.status !== ProcessStatus.WaitingToSend)) {
